# `continuity_log.md` 模板（已对齐 outline 的“收口/代价/换形”要求）
...
| ID | 事实（可追责/不可反悔） | 时间点（章/日期） | 可指认点（见证/物证/记录） | 关联收口（硬/软 + 对应章） | 备注 |
|---|---|---:|---|---|---|
...
| ID | 状态 | 起始章 | 线索（可验证实体） | 挂靠剧情线（D-xx） | 计划（埋/推/收章） | 当前持有人/知情者（可用方式） | 关联收口（硬/软 + 对应章） | 备注（压住/压不住会怎样） |
|---|---|---:|---|---|---|---|---|---|
...
> 要求：此表必须记录“软收口”的策略更新（可验证句式），并与 D/E 表可追溯。

| 人物 | 当前目标（下一步要做什么） | 防御/策略更新（软收口句式：从此在X会Y/不再Z） | 关系债务（欠/被欠：对象+凭据+追讨方式） | 伤疤/不可逆代价（可指认） | 受哪条线咬住（D-xx） | 计时器/触发点（可触发条件） |
|---|---|---|---|---|---|---|
...
> 要求：每章必须至少填入一种“收口”更新（硬或软），并且能在 A/B/C/D 中找到对应落点。

| 章 | 本章收口类型（硬/软） | 对应收口句摘要（1句） | 新增禁改事实（F-xx） | 新增/推进线索（CL-xx） | 策略更新（若软收口，写“从此在X会Y/不再Z”） | 新增不可逆代价（人物/组织） | 推进的剧情线（D-xx） | 本章末尾悬置问题（对齐 D-xx 的 NEXT） |
|---|---|---|---|---|---|---|---|---|
